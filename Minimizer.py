...
from Predictions import PDF_theo, tPDF_theo, GFF_theo, CFF_theo
from Predictions import DVCSxsec_theo, DVCSxsec_cost_xBtQ, DVCSxsec_HERA_theo
...
def save_fitting(fitting: Minuit, file: str):
    # file can be a string or a file object
    names  = np.array(fitting.parameters, dtype='object')
    values = np.array(fitting.values)
    errors = np.array(fitting.errors)
    cov    = np.array(fitting.covariance)

    np.savez(names=names, values=values, errors=errors, cov=cov)
...
def forward_H_fit(Paralst_Unp):
    ...
    save_fitting(fit_forw_H, 'GUMP_Output/H_forward_fit.npz')
...
def forward_E_fit(Paralst_Unp):
    ...
    save_fitting(fit_forw_E, 'GUMP_Output/E_forward_fit.npz')
...
def forward_Ht_fit(Paralst_Pol):
    ...
    save_fitting(fit_forw_Ht, 'GUMP_Output/Ht_forward_fit.npz')
...
def forward_Et_fit(Paralst_Pol):
    ...
    save_fitting(fit_forw_Et, 'GUMP_Output/Et_forward_fit.npz')
...
def off_forward_fit(Paralst_Unp, Paralst_Pol):
    ...
    save_fitting(fit_off_forward, 'GUMP_Output/off_forward_fit.npz')
